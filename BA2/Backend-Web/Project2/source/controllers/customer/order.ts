...
async function placeOrder(req: Request, res: Response, _: NextFunction) {
    return placeOrderSchema.validateAsync(req.body)
        .then(async (items: Array<{ id: number, amount: number }>) => {
            databaseManager.getCustomerByName(req.header("username")?.toLowerCase() ?? "")
                .then(async c => {
                    // This will always return a customer as the route is behind an authentication check
                    databaseManager.placeOrder(c!.id, items)
                        .then(order_id => (
