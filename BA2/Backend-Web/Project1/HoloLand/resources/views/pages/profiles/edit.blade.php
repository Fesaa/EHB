...
@section('form')
    <div class="styled-form-container">
        <form class="styled-form label-left" method="post" action="{{ route('profiles.update', ["profile" => $user->id]) }}"
              enctype="multipart/form-data">
            @csrf
            @method('PUT')

            @if(User::AuthUser()->hasPrivilegeByString('MEMBERS_EDIT_PROFILE'))
                @if(User::AuthUser()->getHighestRole()->outRanks($user->getHighestRole()))
